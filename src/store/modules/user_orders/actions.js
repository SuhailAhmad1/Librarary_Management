export default {
    async placeOrder(context, payload){
        ...
        const response = await fetch(process.env.VUE_APP_BACKEND_URL + "/api/user/place_order", {
            method: 'POST',
            headers: {
    ...
    async setOrderItems(context,payload){
        const response = await fetch(process.env.VUE_APP_BACKEND_URL + "/api/user/get_user_orders", {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${localStorage.getItem("token")}`
            },
        ...
        context.commit('addToOrders', responseData.data)
    },
